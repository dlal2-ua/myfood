...
async def enqueue_supplement_suggestion_job(ai_session_id: str) -> None:
    await enqueue_job(SUPPLEMENT_QUEUE_KEY, ai_session_id)


async def dequeue_supplement_suggestion_job(timeout_seconds: int = 5) -> str | None:
    return await dequeue_job(SUPPLEMENT_QUEUE_KEY, timeout_seconds)
